...
// To make scatter plot work on UEFA directory, edit the follows.
//   1. Move set-up from drawGraph() to the outside as global vars but keep the
//      the assignemnt inside drawGraph().
//   2. Use $('#chart').find('svg').remove(); and $('#legend').find('svg').remove();
//      to remove svg elements.
//   3. Change #body to #chart in d3.select("#body").on("click", resetClick);
...
function drawGraph(xText, yText, rText, cText) {
    ...
    mobileScreen = ($(window).innerWidth() < 500 ? true : false);
    ...
    circleGroup.selectAll("clubs")
        .data(clubs.sort(function(a, b) {
            return b.PossessionScore > a.PossessionScore;
        })) //Sort so the biggest circles are below
        .enter().append("circle")
        .attr("class", function(d, i) {
            return "clubs " + d.ClubCode;
        })
